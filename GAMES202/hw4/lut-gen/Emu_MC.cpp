...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        float F = roughness + (1.0 - roughness) * std::pow(1.0 - NdotV, 5.0);
        float D = DistributionGGX(N, H, roughness);
        float G = GeometrySmith(roughness, dot(N, V), dot(N, L));
        float denominator = 4.0 * std::max(0.0001f, dot(N, L)) * std::max(0.0001f, NdotV);
        float fr = (D * G * F) * std::max(0.0f, dot(N, L)) / denominator / sampleList.PDFs[i];
        A += fr;
        B += fr;
        C += fr;
    }
